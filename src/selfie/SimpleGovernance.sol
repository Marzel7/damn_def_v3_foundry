        ...
        if (!success) {
            if (returndata.length > 0) {
                assembly {
                    ...
                    // @audit returnData points to the beginning of bytes memory which starts with a length prefix 32 bytes long
                    // @audit after that prefix the actual revert data follows
                }
            } else {
