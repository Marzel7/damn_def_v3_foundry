...
contract TestContract is Test {
    ...
    function testFindOwner() public {
        uint256 slot = stdstore.target(address(test)).sig("owner()").find();
        assertEq(slot, 1);
    }

    function testOwnerValue() public {
        uint256 slot = stdstore.target(address(test)).sig("owner()").find();
        assertEq(
            uint256(vm.load(address(test), bytes32(slot))),
